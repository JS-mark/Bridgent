...
pnpm --filter @bridgent/example-01-zod-hello exec bridgent dev ./server.ts
```
...
packages/cli           bridgent — CLI (npm name has no scope)
...
`packages/cli` is published as **`bridgent`** (no scope, for `npx bridgent`). Built by tsdown with banner `#!/usr/bin/env node`; tsdown auto-grants +x on the bin output (no chmod needed — ADR's R5).
