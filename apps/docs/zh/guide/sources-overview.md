...
|---|---|---|---|---|---|---|
| 状态 | 已发布(`@bridgent/core`) | 已发布(`@bridgent/source-openapi`) | 已发布(`@bridgent/source-prisma`) | 路线图 | 路线图 | 路线图 |
| 最小代码 | 每个工具一次 `defineTool` | `await fromOpenApi({ spec })` | `await fromPrisma({ client })` | — | — | — |
| 工具来源 | 每个 `defineTool` 调用 | spec 中的每个 operation | 每个 model × 5 个读方法 | schema 表 | router procedure | resolver 字段 |
| 命名 | 作者提供的 `name` | `operationId`(slug 化)或 `${method}_${path}` | `<namespace?><modelCamel>_<method>` | TBD | TBD | TBD |
| 过滤 | 不适用 | `allow` / `allowOperations` / `denyOperations` / `pathFilter` / `respectExtensions` | `allow` / `modelFilter` / `allowTools` / `denyTools` | TBD | TBD | TBD |
| 鉴权(v0.1) | 由作者在 `run` 中提供 | Bearer(静态或 thunk) | 复用 PrismaClient 的 datasource 凭据 | — | — | — |
| 读 / 写 | 由作者控制 | 默认只读;通过 `allow.mutating` 显式启用 | 只读 5 件套;`mutating: true` v0.1 静默忽略 | — | — | — |
...
- **From Drizzle** —— 与 Prisma 形态一致:schema → 读工具,同样的护栏。
