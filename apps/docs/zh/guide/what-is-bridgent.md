...
**Bridgent AI** 用一行命令把以下任意一种转换为生产可用的 **MCP 服务器**:
...
| `openapi.json` | 每个操作一个 MCP 工具,带鉴权(Bearer / API key / OAuth2 PKCE) |
| `appRouter`(tRPC) | 每个过程一个工具,完整类型 |
...
路线图:Drizzle / tRPC / GraphQL 数据源、写侧 Prisma + 审计日志、品牌化 Inspector UI、托管的 Cloud 版本。
