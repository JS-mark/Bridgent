...
完成后 maintainer 只需：写 changeset → push → GitHub Action 自动开 Version PR → merge 后自动 publish + 创建 release，再按 `docs/launch/` 模板贴 4 个渠道。**真录 demo GIF** 仍是 user 手工，本 plan 不做。
...
| changesets | `@changesets/cli@^2.31` + `@changesets/changelog-github@^0.7` 配置；publishable 包 `linked` 同步版本 | beta channel；snapshot release（v0.5+ 再做） |
| Release workflow | `.github/workflows/release.yml` 跑 `changesets/action`，PR 模式生成 Version PR → 合入后 publish | 双 npm registry 发布；GitHub Packages |
...
4. **`linked` 同步版本**：4 个 publishable 包用 `linked`，alpha 阶段保持版本号同步（用户不需要记多个版本组合）；v0.5+ 再考虑拆开
...
```json
{
  ...
  "commit": false,
  "fixed": [],
  "linked": [["@bridgent/core", "@bridgent/source-openapi", "@bridgent/source-prisma", "bridgent"]],
  "access": "public",
  "baseBranch": "main",
...
| R2 | `linked` 让 4 包同步版本是否影响 `workspace:*` 协议 swap | dry-run 看产出的 package.json |
...
- **ADR-025** changesets `linked` 模式让 4 个 publishable 包同步版本号，alpha 阶段保持简单；v0.5+ 再考虑拆开
