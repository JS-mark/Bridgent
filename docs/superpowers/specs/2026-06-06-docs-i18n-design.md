...
- frontmatter 中的 `link`、`actions[].link` 写裸路径(如 `/guide/getting-started`),VitePress 在 `/zh/` 上下文下自动加前缀
...
| sidebar `link` 写错前缀导致 404 | 统一用裸路径(`/guide/...`),由 VitePress 按 locale 解析,不手写 `/zh/` 前缀 |
| frontmatter 中 `actions[].link` 易遗忘前缀 | 在 spec 中明确"裸路径",中文 home 也写 `/guide/getting-started`,VitePress 会在 `/zh/index.md` 上下文中解析为 `/zh/guide/getting-started` |
